...
def conv(x, c):
    ...
    dilated_size = 1
    if c['block_filters_internal'] == 256 and stride == 1:
        dilated_size = 2
    if c['block_filters_internal'] == 512 and stride == 1:
        dilated_size = 4
    return tf.nn.convolution(x, weights, padding='SAME', strides=[1, stride, stride], dilation_rate=[1,dilated_size, dilated_size])  # tf.nn.conv3d(x, weights, [1, 1, stride, stride, 1], padding='SAME', dilations=[1,1,dilated_size, dilated_size,1])
